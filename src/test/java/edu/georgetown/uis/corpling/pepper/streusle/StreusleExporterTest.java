 ...
 * {@link StreusleExporter} class. Feel free to adapt and enhance this test class
 ...
 * @author Luke Gessler
 */
public class StreusleExporterTest extends PepperExporterTest {
	/**
	...
	@Before
	public void setUp() {
		setFixture(new StreusleExporter());
	...
	public void test_DummyImplementation() {
		...
		getFixture().setCorpusDesc(new CorpusDesc().setCorpusPath(URI.createFileURI(getTempPath("StreusleExporter").getAbsolutePath())));

		// starts the Pepper framework and the conversion process
		start();

		File superCorpus = new File(getTempPath("StreusleExporter").getAbsolutePath() + "/rootCorpus");
